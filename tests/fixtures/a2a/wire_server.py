...
def selected(params):
    ...
    assert not params.metadata and not params.message.metadata and not params.message.extensions
    assert params.message.role == p.ROLE_USER and len(params.message.parts)==1
    assert params.message.parts[0].WhichOneof("content")=="text"
...
class Handler:
    async def on_message_send(self,params,context):
        ...
        reply=value("message_result","message",p.Message);reply.parts[0].text=text
        return reply
    ...
    async def on_cancel_task(self,params,context):
        ...
        return task
app=Starlette(routes=create_jsonrpc_routes(Handler(),"/rpc",enable_v0_3_compat=False))
