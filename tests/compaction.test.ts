...
for(const retain of [0,1])for(const provider of ['vercel','openrouter','open_responses'])for(const trigger of ['threshold','overflow'])test(`CP01/CP02 ${provider} ${trigger} retain=${retain}: CLI and isolated ACP sessions compact real tool history`,{timeout:15000},async()=>{
  ...
  const structured=retain===0&&trigger==='threshold';
  const finalText=structured?'{"answer":"done"}':'done';
  ...
  const gateway=await server(async(req,res)=>{
    ...
    const r=JSON.parse((await body(req)).toString());assert.equal(r.model,model);assert.deepEqual(r.reasoning,{effort:"low"});
    ...
    assert.equal(state.summaries,1);respond({chunks:[finalText]},'final');
  });
  try{
    ...
    assert.equal(cli.outcome.output,finalText);if(structured)assert.equal(cli.output_validation.status,'valid');assert.equal(cli.accounting.model_calls,trigger==='overflow'?'6':'5');assert.equal(cli.accounting.tool_calls,'3');
