...
for(const provider of ['vercel','openrouter','open_responses'])for(const trigger of ['threshold','overflow'])test(`CP01/CP02 ${provider} ${trigger}: CLI and isolated ACP sessions compact real tool history`,{timeout:15000},async()=>{
  ...
  const gateway=await server(async(req,res)=>{
    ...
    if(summarizing){
      ...
      if(provider==='open_responses')assert.deepEqual(messages.filter((m:any)=>m.type==='reasoning').map((m:any)=>m.id),Array.from({length:state.reads-1},(_,i)=>`rs_read${i+1}`));
      respond({chunks:[summary+(effectful?` Artifact: ${state.artifact} was created once.`:'')]},'summary');return;
    }
    if(state.summaries&&!state.after){
      state.after=true;const users=messages.filter((m:any)=>m.role==='user');assert(textOf(users[1]).includes(summary));
      const tools=messages.filter((m:any)=>m.role==='tool'||m.type==='function_call_output');assert.equal(tools.length,1);
      if(provider==='open_responses')assert.deepEqual(messages.filter((m:any)=>m.type==='reasoning').map((m:any)=>m.id),[`rs_read${state.reads}`]);
    }
    if(state.reads<3){state.reads++;const writing=effectful&&state.reads===1;respond({call:{id:`read${state.reads}`,name:writing?'fs_write':'fs_read',arguments:JSON.stringify(writing?{path:state.artifact,text:'written-once',expected_revision:null}:{path:'evidence.txt'})}},`read${state.reads}`);return;}
  ...
  try{
    ...
    const native=await readFile(trace,'utf8');assert(!native.includes(summary));assert(!native.includes('PRIVATE_'));
    ...
    const record=finished[0].compaction;assert.equal(record.trigger,trigger);assert.equal(record.status,'completed');assert(BigInt(record.after_bytes)<BigInt(record.before_bytes));assert.equal(events.filter(e=>e.type==='assistant.text.delta').length,1);
    ...
    await withPablo({binary,args,env:cleanEnv(),onCompaction:n=>{notifications.push(n);},onUpdate:n=>{if(n.update.sessionUpdate==='agent_message_chunk'&&n.update.content.type==='text')answers.push(n.update.content.text);}},async cx=>{
      ...
      for(let i=0;i<2;i++){
        ...
        const captured=await readFile(join(cwd,`acp-${sessionId}.jsonl`),'utf8');assert(captured.includes(summary));assert(!captured.includes('PRIVATE_'));
        ...
        const local=notifications.filter(n=>n.sessionId===sessionId);assert.equal(local.length,2);assert.equal(local[1].summary,summary+(effectful?` Artifact: ${runs.get(`ACP ${i}: inspect evidence in three steps`)!.artifact} was created once.`:''));
