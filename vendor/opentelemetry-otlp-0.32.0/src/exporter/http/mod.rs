...
impl HttpExporterBuilder {
    /// Use explicit values or constants only, including headers and compression.
    /// The default constructor itself performs no environment lookup.
    pub fn without_environment(mut self) -> Self {
        self.ignore_environment = true;
        self
    }
    fn build_client(
        &mut self,
        ...
        let protocol = if self.ignore_environment {
            self.exporter_config.protocol.unwrap_or_default()
        } else { super::resolve_protocol(signal_protocol_var, self.exporter_config.protocol) };
        ...
        let endpoint = if self.ignore_environment {
            build_endpoint_uri(self.exporter_config.endpoint.as_deref().unwrap_or("http://localhost:4318/v1/traces"), "")?
        } else { resolve_http_endpoint(
            signal_endpoint_var,
        ...
        let timeout = if self.ignore_environment { self.exporter_config.timeout.unwrap_or(std::time::Duration::from_secs(10)) } else { resolve_timeout(signal_timeout_var, self.exporter_config.timeout.as_ref()) };
        ...
        if let Some(input) = (!self.ignore_environment)
            .then(|| env::var(signal_http_headers_var).or_else(|_| env::var(OTEL_EXPORTER_OTLP_HEADERS)))
            .and_then(Result::ok)
        {
    ...
    fn resolve_compression(
        ...
        if self.ignore_environment { Ok(self.http_config.compression) } else { super::resolve_compression_from_env(self.http_config.compression, env_override) }
    }
