...
impl BatchSpanProcessor {
    ...
    /// Construct using an explicit config without first reading environment defaults.
    pub fn builder_with_config<E>(exporter: E, config: BatchConfig) -> BatchSpanProcessorBuilder<E>
    where E: SpanExporter + Send + 'static,
    {
        BatchSpanProcessorBuilder { exporter, config }
    }
...
impl BatchConfigBuilder {
    /// SDK constants only; no environment lookup. Programmatic setters still apply.
