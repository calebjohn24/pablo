...
try {
  ...
  const report={checkpoint:'C2.5',timestamp:new Date().toISOString(),platform:`${process.platform}/${process.arch}`,source_sha256:await sourceFingerprint(root),binary_sha256:createHash('sha256').update(await readFile(binary)).digest('hex'),method:{samples:30,warmup:5,cache:'warm filesystem; no forced eviction',prompt:'two loopback HTTP/SSE calls plus one real filesystem tool and ACP envelope delivery; fresh sessions in reused process',tool:'native tool.started to tool.finished timestamps; includes joined worker and filesystem work',search_bytes:1048700,setup:'fixture construction and process/catalog/SDK setup excluded from measured warm samples',baseline:'new capability workload; no equivalent C1 native filesystem tool'},measurements};
