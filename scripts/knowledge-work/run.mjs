...
function options(argv) {
  const o={harnesses:'pablo,pablo-astra,codex,claude,ori,pi',seeds:'42',tasks:'all',repeats:'1',timeout:'180',output:'.pablo/measurements/knowledge-work',live:false};
  for(let i=0;i<argv.length;i++) {const k=argv[i].replace(/^--/,'');if(k==='live')o.live=true;else if(Object.hasOwn(o,k))o[k]=argv[++i];else throw Error(`Unknown option ${argv[i]}`);}
...
export async function main(argv=process.argv.slice(2)) {
  ...
  try {
    ...
    const sourceFiles=['tasks.mjs','adapters.mjs','run.mjs','monitor.py'];
    ...
    trials: for(let rep=0;rep<o.repeats;rep++)for(const task of selected) {
      ...
      for(const h of order) {
        ...
        const row={id,harness:h,model:models[h],task:task.id,seed:task.seed,repeat:rep,fingerprint:task.fingerprint,status:'unavailable',resources:null,correctness:null};
        if(!builds[h].unavailable) {
          ...
          try {
            await launch(spec,env);row.resources=JSON.parse(await readFile(join(artifacts,'resources.json'),'utf8'));
    ...
    const table=['| Harness / model | Passes | Facts | Citations | Completed-task wall p50 s | Peak tree MiB | CPU p50 s |','|---|---:|---:|---:|---:|---:|---:|'];
    const fmt=(n,d=1)=>n==null?'—':n.toFixed(d);
    for(const[h,s]of Object.entries(report.summary))table.push(`| ${h} / ${models[h]} | ${s.passed}/${s.attempts} | ${fmt(s.mean_factual_score*100)}% | ${fmt(s.mean_citation_score*100)}% | ${fmt(s.completed_wall_p50_ms==null?null:s.completed_wall_p50_ms/1000)} | ${fmt(s.peak_tree_rss_kib==null?null:s.peak_tree_rss_kib/1024)} | ${fmt(s.cpu_p50_s,2)} |`);
