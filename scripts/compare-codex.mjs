...
class Peer{
 ...
 async init(){if(this.name==='pablo')await this.request('initialize',{protocolVersion:1,clientCapabilities:{_meta:{'pablo/v2':true}}});else{await this.request('initialize',{clientInfo:{name:'pablo_comparison',version:'1'},capabilities:{experimentalApi:true}});this.send('initialized',{});}return performance.now()-this.started;}
 async task(){const before=performance.now();let id;if(this.name==='pablo')id=(await this.request('session/new',{cwd,mcpServers:[]})).sessionId;else id=(await this.request('thread/start',{cwd,model:'gpt-5.4',modelProvider:'fixture',sandbox:'read-only',approvalPolicy:'never',ephemeral:true})).thread.id;
  ...
  if(this.name==='pablo'){const r=await this.request('session/prompt',{sessionId:id,prompt:[{type:'text',text:task}]});assert.equal(r._meta['pablo/v2'].outcome.output,'benchmark');}
