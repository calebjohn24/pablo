...
Defaults permit four model calls, two tool calls, 120 seconds per run, 30 seconds per tool, and 64 KiB each for tool arguments and the complete serialized tool result. Requested tool limits can narrow the host limits. A separate 256 KiB request-context bound covers serialized instructions, messages, and tool definitions. No tool runs without model-call budget left to consume its result.
