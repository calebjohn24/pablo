...
C3.21 extracts official stable ACP initialize/new/prompt/cancel handlers for direct in-memory dispatch and stdio. Typed updates require consumption acknowledgements and owned close joins the same runtime worker. Freeze depth-one identity, narrowed authority and root-atomic capacity/accounting before C3.22 execution; do not advertise delegation at the contract stage. C3.22 scopes child routes/catalogs on PreparedRun and keys host reuse by the narrowed scope to prevent broader cached resources from resurfacing. Bound child dispatch retains admitted input/workspace/trace parent and root accounting; shared capacity survives worker cleanup and cancelled close callers until the owner observes the join. The internal supervisor owns FIFO handles/results outside model history and settles stop only after joined cleanup; RootOwner adds the policy-checked tool and joins children before native root terminal accounting; the root ledger now binds native/ACP/OTel attribution once per registered execution, while decoded identity grants no authority. Scoped shell permits and conservative child MCP capacity now remain held through joined cleanup; root retained context and child-local growth now share admitted capacity, with compaction reducing accepted history only after its boundary. Native event counts now reserve run terminals and operation start/finish pairs before execution, allowing closings after admission shuts. Traced trees now admit projected bytes and event counts atomically with prepaid run-terminal space; untraced trees skip byte serialization. Supervision now forwards original native events before ACP projection/coalescing with consumption acknowledgements and joined receiver-loss cleanup. A single shared consumer now assigns root_seq while preserving native seq, validates bound sources and reserves projection overhead without double charge. Cancellation now drains healthy native closings before root settlement, with a bounded cancelled receipt window and joined stalled-consumer cleanup. Root-factory MCP capacity now precedes setup and remains held through manager-owned tool cleanup, including abandoned close callers; configured CLI/ACP roots now use the shared factory/consumer with explicit children activation, root-only results and agent-scoped ACP tool-call IDs. A02 now passes configured execution, stop, root cancellation/deadline/normal completion, startup/trace failures and exported-span correlation; final one-child workload measurements are recorded. C3.23 now polls two owned child executions, with FIFO queueing and a joined drain before root cleanup; A03 barriers prove overlap, sibling isolation, root cancellation and shared-budget races. See [A03 acceptance](evidence/c3.23-a03.md). C3.24 resolves validated handoffs from retained owned results (D055). See [A02 acceptance](evidence/c3.22-a02.md). See [configured host evidence](evidence/c3.22-configured-hosts.md) and [root factory evidence](evidence/c3.22-root-factory.md). See [cancellation evidence](evidence/c3.22-cancellation-drain.md), [root consumer evidence](evidence/c3.22-root-consumer.md), [native delivery evidence](evidence/c3.22-native-delivery.md), [trace budget evidence](evidence/c3.22-trace-budget.md), [event budget evidence](evidence/c3.22-event-budget.md), [context evidence](evidence/c3.22-resident-context.md), [capacity evidence](evidence/c3.22-process-capacity.md), [attribution evidence](evidence/c3.22-attribution.md), [root ownership evidence](evidence/c3.22-root-owner.md), [supervision evidence](evidence/c3.22-supervisor.md), [bound dispatch evidence](evidence/c3.22-bound-acp.md), [child contract](contracts/c3-children.md) and [admission evidence](evidence/c3.22-child-admission.md).

### D055 — Select owned results, verify artifact revisions at promotion
C3.24 selects immutable source/result IDs from schema-valid completed snapshots. Only selected structured output and validation/accounting/trace receipts enter ordinary child context; source transcripts and authority never transfer. Retained source snapshots and downstream selections survive compaction. Artifact path/revision checks use narrowed physical filesystem read authority immediately before downstream setup, including after queue waits; streaming verification honors explicit file limits and joined cancellation, without default work caps or a concurrent-write snapshot claim. E01 now proves MCP/Skill/active-child/completed-result ownership through forced compaction, Rust embedding/ACP parity, Collector identity/privacy/outage and refreshed live defaults; see [combined acceptance](evidence/c3.25-e01.md). See [handoff contract](contracts/c3-children.md#authority-context-and-shared-workspace).

### D056 — A2A cards describe peers without granting authority
C3.26 pins A2A 1.0 JSONRPC and the independent Python SDK. Explicit named HTTPS card/RPC tuples intersect host ceilings; public card fetching never redirects or sends credentials. Bearer references use a separate consumer scoped only to the configured RPC endpoint. Bound raw card validation and retain its digest as untrusted metadata, not authentication. See [A2A contract](contracts/c3-a2a.md); named retrieval now creates owned local remote proxies, and the ledger preserves their kind. Remote IDs bind separately without replacing local sessions. Bounded wire codecs now round-trip the pinned SDK JSONRPC/SSE dispatcher; bounded file Parts and inert URL references preserve explicit data without filesystem authority. Negotiated trace receipts cannot reparent local execution; generic peers omit the optional extension. See [R01 evidence](evidence/c3.26-r01.md) for acceptance. C3.27 integrates bounded transport/assembly with shared supervisor admission, explicit host/model Parts and acknowledged native/ACP events. Decimal-string remote usage stays separate from local enforced accounting; cancellation receipts never claim rollback. [R02](evidence/c3.27-r02.md) and [R03](evidence/c3.28-r03.md) pass: CLI/ACP cancellation/loss/input-required boundaries preserve local truth, and an independent OTel peer proves real Collector parenting, generic fallback and outage parity. No hard remote spending or service-stop guarantee is implied.
