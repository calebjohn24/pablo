...
C3.26 pins A2A 1.0 JSONRPC and the independent Python SDK. Explicit named HTTPS card/RPC tuples intersect host ceilings; public card fetching never redirects or sends credentials. Bearer references use a separate consumer scoped only to the configured RPC endpoint. Bound raw card validation and retain its digest as untrusted metadata, not authentication. See [A2A contract](contracts/c3-a2a.md); named retrieval now creates owned local remote proxies, and the ledger preserves their kind. Remote IDs bind separately without replacing local sessions. Remaining wire mapping proof keeps R01 in progress.
