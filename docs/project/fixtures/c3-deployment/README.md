...
These C3.1 fixtures specify the [contract](../../contracts/c3-deployment-config.md) and [inventory](../../contracts/c3-deployment-options.md). C3.2 runs them through `pablo_core::deployment`; executable configuration commands and runtime activation remain C3.3. The source [schema](../../schemas/deployment-v1.schema.json) validates document shape; the Rust resolver also enforces semantic requirements. [State](../../state.json) alone records checkpoint progress.
...
| [Case manifest](cases.json) | Eighteen concrete parser/schema cases and thirty G02/G03 descriptions; 21 G02 cases link passing Rust tests/evidence, while nine G03 interface/runtime cases remain `not_run` |
...
C3.3's planned additional commands remain unimplemented:
