...
Use C1 measurements to prioritize investigations only when they affect this slice. Metadata JSONL serialization is visible in the CPU-only stream fixture; benchmark realistic tasks before changing its ownership or format. Differences in process startup, setup, shell execution and transport need separate attribution before treating an ACP-to-core ratio as pure protocol cost.
