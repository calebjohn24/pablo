...
| `model.provider` | Enum `vercel` (default), `openrouter`, `open_responses` | C3.5 selection/inspection; C3.6 OpenRouter and C3.9 Open Responses streaming execution. [Provider contract](c3-provider-selection.md) defines adapter availability and capabilities |
...
| `model.endpoint` | Gateway-owned literal chat-completions URL, or explicit Open Responses HTTPS URL | Vercel `https://ai-gateway.vercel.sh/v1/chat/completions`; OpenRouter `https://openrouter.ai/api/v1/chat/completions`. Omission follows gateway selection; arbitrary gateway endpoints reject. Open Responses requires a complete explicit HTTPS endpoint with no userinfo/query/fragment |
...
| `output.repair.enabled`, `output.repair.max_feedback_bytes` | `false`; 4096 bytes, range 512–4096 | C3.14 [J02](c3-output-repair.md): one same-conversation repair with shared budgets; locked overrides only disable/narrow |
...
C3.26 adds `a2a.remotes` (empty by default), with required `card_url`/`endpoint`, optional `bearer.scheme`/`bearer.credential`, and `trace_context` default false. Immutable `authority[].a2a_remotes` intersects exact named URL pairs. `a2a.bearer` credentials are scoped to the configured RPC endpoint. See [the A2A contract](c3-a2a.md); configured fetch/proxy identity is implemented but remaining wire mapping proof and full R01 remain pending, and configuration alone does not enable task delegation.
