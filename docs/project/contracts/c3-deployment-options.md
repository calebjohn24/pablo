...
| --- | --- | --- |
| `context` and model-profile context capacity | C3.12a | Basic automatic compaction, declared usable capacity, conservative estimate/safety margin, one summary allowance, bounded summary output and recent complete turns; one overflow recovery; all work within root ceilings and content policy |
