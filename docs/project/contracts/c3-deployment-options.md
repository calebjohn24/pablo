...
| `model.reasoning`, `model.reasoning_capabilities` | Optional effort string or `{budget_tokens=N}`; optional declared efforts/mandatory/budget support | C3.33c [reasoning contract](c3-reasoning-latency.md). Omission preserves provider default. Named route equivalents: `models.NAME.model_options.reasoning` and `models.NAME.capabilities.reasoning` |
...
| `--reasoning-effort`, `--reasoning-budget-tokens` | Mutually exclusive `model.reasoning` overrides; configured routes retain per-entry settings |
