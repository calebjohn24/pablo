...
C3.26 adds `a2a.remotes` (empty by default), with required `card_url`/`endpoint`, optional `bearer.scheme`/`bearer.credential`, and `trace_context` default false. Immutable `authority[].a2a_remotes` intersects exact named URL pairs. `a2a.bearer` credentials are scoped to the configured RPC endpoint. See [the A2A contract](c3-a2a.md); configured fetch/proxy integration and full R01 remain pending, and configuration alone does not enable task delegation.
