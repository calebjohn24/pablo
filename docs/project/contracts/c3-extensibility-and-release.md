...
- Tool catalogs and trusted instruction prefixes stay stable within a run. Newly activated Skill instructions enter at a documented message boundary; child overlays follow shared instructions. Ordered configured model fallback is the only new retry owner; it never replays tools or restarts the task. Dynamic catalog churn, adaptive model routing and general parallel tool dispatch remain deferred.
