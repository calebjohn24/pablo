...
C3.10 adds `PABLO_MEASURE_ROUTED=1` to both harnesses for an explicit single-entry route. It uses the same selected provider/model, real tool workload and limits, so ordinary deployment versus named-route resolution can be compared without exercising future fallback.
