//! Shared per-run MCP protocol and normalized tool contract.
use super::{
...
pub struct Session {
    ...
    child: Option<Child>,
    pid: Option<Pid>,
    http: Option<Arc<super::http_transport::HttpState>>,
    armed: bool,
    cleanup_failed: bool,
...
impl Session {
    ...
    pub(crate) async fn start(
        ...
        session
            .initialize(transport, startup_deadline, cancellation)
            .await?;
        Ok(session)
    }
    pub(crate) async fn start_http(
        ...
        let transport = super::http_transport::HttpTransport::new(
            fixture.unwrap_or(url),
            headers,
            fixture.is_some(),
        )
        ...
        let mut session = Self {
            ...
            child: None,
            pid: None,
            http: Some(transport.state.clone()),
            armed: true,
            cleanup_failed: false,
            stderr: None,
            transport_cancel: transport.state.cancellation.clone(),
            bounds: transport.state.bounds.clone(),
            tools: Vec::new(),
            operation_timeout: Duration::from_millis(*operation_timeout_ms),
            catalog_bytes: 0,
        };
    ...
    async fn initialize(
        &mut self,
        transport: impl rmcp::transport::Transport<RoleClient> + 'static,
    ...
    pub fn process_id(&self) -> Option<u32> {
        self.pid.map(|pid| pid.as_raw_nonzero().get() as u32)
    }
    pub(crate) fn decorate(&self, context: &opentelemetry::Context) {
        if let Some(http) = &self.http {
            context.span().set_attributes([
                KeyValue::new("network.transport", "tcp"),
                KeyValue::new("server.address", http.address().to_owned()),
                KeyValue::new("server.port", i64::from(http.port())),
            ]);
        } else if let Some(pid) = self.process_id() {
            context.span().set_attributes([
                KeyValue::new("network.transport", "pipe"),
                KeyValue::new("pablo.mcp.process.id", i64::from(pid)),
            ]);
        }
    }
    pub fn remote_completion_uncertain(&self) -> bool {
        self.http
            .as_ref()
            .is_some_and(|http| http.uncertain.load(std::sync::atomic::Ordering::Acquire))
    }
    ...
    pub(crate) async fn call_with_context(
        ...
        let response = match response {
            ...
            Err(error) => {
                ...
                if let Some(http) = &self.http {
                    if self.remote_completion_uncertain() {
                        http.cancel_request(
                            &id,
                            (Instant::now() + Duration::from_millis(50)).min(cleanup_deadline),
                        )
                        .await;
                    }
                } else if matches!(error, Error::Cancelled | Error::TimedOut) {
                    let notification =
    ...
    async fn close_before(&mut self, deadline: Instant) -> Result<(), Error> {
        ...
        if let Some(http) = &self.http {
            ...
            if let Some(mut service) = self.service.take() {
                joined = service.close().await.is_ok();
            }
            let closed = http.teardown(deadline).await.is_ok();
            self.armed = false;
            self.cleanup_failed = !joined || !closed;
            return if self.cleanup_failed {
                Err(Error::Cleanup)
            } else {
                Ok(())
        ...
        let killed = matches!(
            kill_process_group(self.pid.unwrap(), Signal::KILL),
        ...
        let joined = timeout_at(deadline, async {
            ...
            loop {
                match rustix::process::test_kill_process_group(self.pid.unwrap()) {
...
mod tests {
    ...
    async fn cancellation_and_invalid_arguments_do_not_leave_owned_work() {
        ...
        let cancellation = CancellationToken::new();
        let deadline = Instant::now() + Duration::from_secs(5);
        let mut session = Session::start(&server, &cwd, BTreeMap::new(), deadline, &cancellation)
            .await
            .unwrap();
        assert_eq!(
            session
        ...
        session.close().await.unwrap();
        assert_eq!(
            rustix::process::test_kill_process_group(session.pid.unwrap()),
            Err(rustix::io::Errno::SRCH)
        );
        std::fs::remove_dir_all(cwd).unwrap();
    }
